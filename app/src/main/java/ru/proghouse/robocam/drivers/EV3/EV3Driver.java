...
public class EV3Driver extends RoboCamDriver {
    ...
    private void storeJoystickCoordinates() {
    ...
    public String getUsedKeys() {
        ...
        for (EV3KeyGroup keyGroup : keyGroups) {
            if (keyGroup.isActive()) {
                keys.addAll(keyGroup.getUpKeyCodes());
                keys.addAll(keyGroup.getLeftKeyCodes());
                keys.addAll(keyGroup.getDownKeyCodes());
                keys.addAll(keyGroup.getRightKeyCodes());
    ...
    private void loadOutputPorts(Element parentNode, EV3Controller controller) {
    ...
    public void loadSettingsFromXml(Context context, File file, Document xml) throws Exception {
        ...
        for (int i = 0; i < joystickNodes.getLength(); i++) {
            ...
            if (joystickIndex >= 0 && joystickIndex < joysticks.size()) {
                ...
                if (joystick.getType() != JOYSTICK_TYPE_MAILBOX)
                    loadOutputPorts(joystickNode, joystick);
            }
        }
        ...
        for (int i = 0; i < keyGroupNodes.getLength(); i++) {
            ...
            EV3KeyGroup keyGroup = new EV3KeyGroup();
            keyGroup.setType(StringHelper.intFromString(keyGroupNode.getAttribute("Type"),
                    RoboCamDriver.JOYSTICK_TYPE_INDEPENDENT_MOTORS));
            keyGroup.setActive(StringHelper.booleanFromString(keyGroupNode.getAttribute("Active"), false));
            keyGroup.setMailbox(StringHelper.stringFromString(keyGroupNode.getAttribute("Mailbox"), ""));
            keyGroup.setBehavior(0, StringHelper.intFromString(keyGroupNode.getAttribute("Behavior0"),
                    RoboCamDriver.JOYSTICK_BEHAVIOR_RETURN_TO_ZERO));
    ...
    private void restoreDefaultSettings() {
        ...
        keyGroups.get(0).setType(JOYSTICK_TYPE_STEERING);
        keyGroups.get(0).getUpKeyCodes().add(87);//W
        keyGroups.get(0).getUpKeyCodes().add(38);//up arrow
        keyGroups.get(0).getLeftKeyCodes().add(65);//A
        keyGroups.get(0).getLeftKeyCodes().add(37);//left arrow
        keyGroups.get(0).getDownKeyCodes().add(83);//S
...
//        }
    ...
    private void startReadingControllerValues() {
        new Thread(new EV3ControllerValuesReader()).start();
    }
